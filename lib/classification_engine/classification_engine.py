...
class ClassificationEngine(object):
    ...
    def _gen_csv(self, pill_label_path, pill_image_path, pres_label_path, save_dir):
        ...
        dataframe.to_csv(os.path.join(save_dir, "processed_data"+".csv"), index=False)
        return dataframe


    def run_state(self, pill_label_path, pill_image_path, pres_label_path, save_dir):
        if os.path.isfile(os.path.join(save_dir, "processed_data"+".csv")):
            print('Loading csv file ..')
            dataframe = pd.read_csv(os.path.join(save_dir, "processed_data"+".csv"))
        else:
