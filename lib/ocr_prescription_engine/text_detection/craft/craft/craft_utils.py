...
def getDetBoxes_core(textmap, linkmap, text_threshold, link_threshold, low_text, check_func=np.max, log_dir=None, debug=False):
    # prepare data
    ...
    if debug:
        cv2.imwrite(os.path.join(log_dir, 'text_map_thresh.png'), text_score*255)
        cv2.imwrite(os.path.join(log_dir, 'link_map_thresh.png'), link_score*255)

    text_score_comb = np.clip(text_score + link_score, 0, 1)
    if debug:
        cv2.imwrite(os.path.join(log_dir, 'text_link_map_thresh.png'),
                text_score_comb*255)
    ...
    for k in range(1, nLabels):
        ...
        if size < 10:
            continue
        # thresholding
        if check_func(textmap[labels == k]) < text_threshold:
            continue
...
def getDetBoxes(textmap, linkmap, text_threshold, link_threshold, low_text, poly=False, check_func=np.max, log_dir=None, debug=False):
    boxes, labels, mapper = getDetBoxes_core(
        textmap, linkmap, text_threshold, link_threshold, low_text, check_func=check_func, log_dir=log_dir, debug=debug)
